...
router.get("/", async (req, res) => {
  try {
    ...
    const [accommodations, restaurants, shops, messes] = await Promise.all([
      AccommodationListing.find({ status: "approved" }).populate(
        "owner",
        "name email"
      ),
      RestaurantListing.find({ status: "approved" }).populate(
        "owner",
        "name email"
      ),
...
router.get("/pending", auth, async (req, res) => {
  try {
    ...
    const [
      ...
      AccommodationListing.find({ status: "pending" }).populate(
        "owner",
        "name email"
      ),
      RestaurantListing.find({ status: "pending" }).populate(
        "owner",
        "name email"
      ),
    ...
    const allPendingListings = [
      ...pendingAccommodations.map((listing) => ({
        ...listing.toObject(),
        type: "accommodation",
      })),
      ...pendingRestaurants.map((listing) => ({
        ...listing.toObject(),
        type: "restaurant",
      })),
      ...pendingShops.map((listing) => ({
        ...listing.toObject(),
        type: "shop",
      })),
      ...pendingMesses.map((listing) => ({
        ...listing.toObject(),
        type: "mess",
      })),
...
router.get("/:type", async (req, res) => {
  try {
    ...
    switch (type) {
      case "accommodations":
        listings = await AccommodationListing.find({
          status: "approved",
      ...
      case "restaurants":
        ...
        break;
      case "shops":
        listings = await ShopListing.find({ status: "approved" }).populate(
          "owner",
          "name email"
        );
        break;
      case "messes":
        listings = await MessListing.find({ status: "approved" }).populate(
          "owner",
          "name email"
        );
        break;
...
router.get("/search", async (req, res) => {
  try {
    ...
    if (!q || q.trim() === "") {
      const allListings = await AccommodationListing.find({
        status: "approved",
...
router.get("/:type/:id", async (req, res) => {
  try {
    ...
    const listing = await Model.findById(id).populate(
      "owner",
      "name email phone"
    );
...
router.get("/restaurants", async (req, res) => {
  try {
    const listings = await RestaurantListing.find({
      status: "approved",
    }).populate("owner", "name email");
...
router.get("/shops", async (req, res) => {
  try {
    const listings = await ShopListing.find({ status: "approved" }).populate(
      "owner",
      "name email"
    );
    res.json({ success: true, listings });
...
router.get("/accommodations", async (req, res) => {
  try {
    const listings = await AccommodationListing.find({
      status: "approved",
    }).populate("owner", "name email");
    res.json({ success: true, listings });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Error fetching accommodations" });
  }
});

router.get("/messes", async (req, res) => {
  try {
    const listings = await MessListing.find({ status: "approved" }).populate(
      "owner",
      "name email"
    );
    res.json({ success: true, listings });
...
router.get("/accommodations/:id", async (req, res) => {
  try {
    const listing = await AccommodationListing.findById(req.params.id).populate(
      "owner",
      "name email phone"
    );
